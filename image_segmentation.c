...
unsigned char im[SIZ][SIZ],copy1[SIZ][SIZ],copy2[SIZ][SIZ];
int x,y,lmin,lmax;
float w;
...
int generate_histogram (long y_interval,long hist[FREQ])
   ...
   {
      float u[1000],a,c,xa,ya,za,bm[1000],fuzzy_ent[1000],f_ent,s_ent[1000],la,lb;
      int i,j=0,xy,nofe,b;
      float fuzzy_entropy_threshold; 
      float fe_max,th;
      float sum_s_ent;
      float x2,x1,sum_x1x2,u_ideal[1000],f_corr,fuzzy_corr,fuzzy_correlation[1000],nofc,bcorr[1000],fuzzy_correlation_threshold;
...
//copy1[SIZ][SIZ]=copy[SIZ][SIZ];
threshold(im,fuzzy_entropy_threshold);
 printf("\n----------------AFTER FUZZY ENTROPY THRESHOLDING----------------------\n");
     creat_image(im);
...
//      printf("\nX1=%f\n",x1);
     ...
     {
       for(i=lmin;i<=lmax;i++)
        f_corr=f_corr+((u_ideal[i]-u[i])*(u_ideal[i]-u[i])*d[i]);
